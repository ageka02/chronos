...
.outeraing {
    ...
    <title><?php echo "[Chronos]Balance_".$_GET['line']."_".$deptaing."_".date('Y_md',strtotime($_GET['tgl']))."_".date('H:i'); ?></title>
...
<body>
    ...
    <div id="right-panel" class="right-panel">
        ...
        <div class="breadcrumbs">
            <div class="col-sm-8">
                <h4 style=""> <i class="fa fa-pencil-square-o bg-warning p-3"></i> Data Actual & Balance <?php echo $_GET['line']." ".$deptaing; ?> </h4>
            </div>
            <div class="col-sm-4">
                <div class="page-header float-right">
                    <div class="">                    	
                        <h1> <?php echo date('l, d F Y',strtotime($_GET['tgl'])); ?> </h1>
                    </div>                    
                </div>
            </div>            
        </div> 
			 		...
			 		<table id="balance_table" width="100%" class="table table-bordered table-hover display compact ">
                            ...
                            $total_order = $total_in = $total_out = $total_sumarize_in = $total_sumarize_out = 0;
		    	 			foreach ($data_table as $value) {
		    	 				...
		    	 				<tr>
                                    ...
                                    <td><?php echo round($value['total_out']-$value['qty_order']); ?></td>
			    	 			</tr>
		    	 				<?php
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <th><?php echo $total_sumarize_out-$total_order; ?></th>
                            </tr>
                        </tfoot>
			    	 </table>
	    	 	</div>
    ...
    <script>
    	jQuery(document).ready(function() {    
            ...
            //         exportOptions: {
            //             format: {
            //                 header: function ( thead,data, row, column, node ) {
            //                     // Strip $ from salary column to make it numeric
            //                     return column === 4 ?
            //                         thead.replace(/e/gi, "entog" ) :
            //                         thead;
            //                 }
            //             }
            //         }
          ...
          jQuery('#balance_table').DataTable({
            ...
            customize: function( xlsx ) {
                            ...
                            jQuery('row:last c', sheet).attr( 's', '22' );
                            jQuery('c[r=I2] t', sheet).text( 'Current IN' );
                            jQuery('c[r=J2] t', sheet).text( 'Current OUT' );
                            jQuery('c[r=K2] t', sheet).text( 'Summarize IN' );
                            jQuery('c[r=L2] t', sheet).text( 'Summarize OUT' );
                        }
             },
